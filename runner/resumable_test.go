...
func (r *failingVideoRunner) Run(ctx context.Context, req ports.VideoGenerationRequest) (*ports.VideoResponse, error) {
...
func threeCutRecipe() *ports.VideoRecipe {
	return &ports.VideoRecipe{
		ProjectTitle: "partial",
		Cuts: []ports.Cut{
			{CutIndex: 1, AudioSync: ports.AudioSync{DurationSec: 8}, KeyframeResult: ports.KeyframeResult{KeyframeReference: "gs://images/cut_1.png"}, ChainControl: ports.ChainControl{IsChainStart: true}},
			{CutIndex: 2, AudioSync: ports.AudioSync{DurationSec: 8}, KeyframeResult: ports.KeyframeResult{KeyframeReference: "gs://images/cut_2.png"}, ChainControl: ports.ChainControl{IsChainStart: true}},
			{CutIndex: 3, AudioSync: ports.AudioSync{DurationSec: 8}, KeyframeResult: ports.KeyframeResult{KeyframeReference: "gs://images/cut_3.png"}, ChainControl: ports.ChainControl{IsChainStart: true}},
		},
	}
}
...
func TestVideoTimelineRunner_RunReturnsPartialResultsOnFailure(t *testing.T) {
	...
	video := &failingVideoRunner{failFromCutIndex: 3, failErr: boom}
	...
	if recipe.Cuts[0].Status != ports.CutStatusGenerated || recipe.Cuts[1].Status != ports.CutStatusGenerated {
		t.Error("completed cuts must be marked generated for resume")
	}
	if recipe.Cuts[2].Status != ports.CutStatusFailed {
...
func TestVideoTimelineRunner_ObserverStopsRun(t *testing.T) {
	...
	runner := NewVideoTimelineRunner(&mockCutKeyframeRunner{}, &mockVideoRunner{}).
		WithCutObserver(func(_ context.Context, cut *ports.Cut, _ *ports.VideoResponse) error {
	...
	if recipe.Cuts[1].Status != ports.CutStatusGenerated {
...
func (g *partialCutImageGenerator) Execute(_ context.Context, cuts []ports.Cut) ([]*ports.KeyframeImage, error) {
	images := make([]*ports.KeyframeImage, len(cuts))
	if len(cuts) > 0 {
		images[0] = &ports.KeyframeImage{Data: []byte("img-1"), MimeType: "image/png"}
	}
	return images, g.err
}

// TestCutKeyframeRunner_RunAndSavePersistsPartialResults は、キーフレーム生成が途中で
// 失敗しても、成功した分の画像とメタデータが保存されてからエラーが返ることを検証します。
// 支払い済みの成果物を保存しておけば、再実行は失敗したカットだけを続きから生成できます。
func TestCutKeyframeRunner_RunAndSavePersistsPartialResults(t *testing.T) {
	...
	recipe := &ports.VideoRecipe{
		...
		Cuts: []ports.Cut{
			{CutIndex: 1, AudioSync: ports.AudioSync{DurationSec: 8}},
			{CutIndex: 2, AudioSync: ports.AudioSync{DurationSec: 8}},
		},
	}

	got, err := r.RunAndSave(context.Background(), recipe, "gs://bucket/jobs/j1/")
	if !errors.Is(err, boom) {
		t.Fatalf("error = %v, want the generation failure", err)
	}
	if got == nil {
