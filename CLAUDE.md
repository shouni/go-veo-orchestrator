...
The Go version lives in `go.mod` (currently **1.27**) — `setup-go` reads it from there, so bump `go.mod` rather than hardcoding a version elsewhere. Note the CI job also pins `golangci-lint` to v2.13.1: v2.12.2 is built with go1.26.1 and refuses a 1.27 target.

**The directive is minor-only on purpose — do not re-add the patch level.** It was `1.26.6` for a while, because `go 1.26` alone let a local 1.26.5 toolchain build the repo with four reachable stdlib CVEs (net/url, crypto/tls, encoding/asn1, net/http) that `govulncheck` reports and no module update can fix. That was reverted: a patch-level directive in a *library* pushes the same toolchain floor onto every consumer, and ap-mv and ap-voice ended up carrying 1.26.6 purely because this module asked for it. Nothing in the graph needs it — `go mod tidy` leaves the minor in place. Keeping a developer's own toolchain current is that developer's job, and CI's `govulncheck` is what actually catches a stale one.

## Architecture

Six packages, strict one-way dependency: `video → ports → veo → {internal/keyframe, runner} → workflow`. `video` depends on nothing in-repo; `workflow` depends on everything. Never import `runner`, `internal/keyframe` or `workflow` from `video`, `ports` or `veo`.

**Five packages are public**: `video`, `ports`, `veo`, `runner`, `workflow`. `internal/keyframe` stays hidden because no consumer builds a keyframe generator directly.

`runner` was under `internal/` while ap-mv reached everything through `workflow.New` and the `ports` interfaces. It came out when ap-mv needed the per-cut hooks: `ports.VideoTimelineRunner` is `Run` alone, so `WithRequestBuilder` / `WithCutGate` / `WithCutObserver` were unreachable, and ap-mv had reimplemented the whole generation loop instead. **The hooks are per-runner, not per-run, so they must only be set on a runner the caller constructed itself** — setting one on the shared `Workflows.Video` would leak one job's closure into another job's run. That is why the answer here was to publish the constructor rather than to widen the `ports` interface: a consumer that needs hooks needs its own instance anyway.
...
runner/    Concrete Runner implementations: VideoScriptRunner, CutKeyframeRunner,
           VideoTimelineRunner (+ VideoRequestBuilder, CutGate, CutObserver),
           VideoPublisherRunner.
...
`video.CutReferenceImages(cut, characters)` is the single rule for building `referenceImages` (`[character art, keyframe]`, max 3, blanks skipped). `DefaultVideoRequestBuilder` and any caller classifying a cut must both use it, so the duration a cut is rounded to always matches the mode its request actually resolves to. The character art is picked with `Character.ReferenceURLFor(cut.AspectRatio)`, the same rule `keyframe.Generator` uses for the still image — pinning `ReferenceURL` here instead would put a ratio-matched keyframe and a ratio-mismatched character sheet side by side in one Veo request, which is exactly the drift the per-ratio variants exist to prevent.
...
- `github.com/shouni/genai-kit` — the single AI dependency, replacing `go-gemini-client` and `gemini-image-kit` (both of which this repo used until the swap; their history explains most of the notes above).
  - `gemini` — AI clients are taken as `gemini.Generator` for both script and image generation: the narrowest genai-free interface, one method (`Generate`). Nothing in this repo imports `google.golang.org/genai`. Structured output uses plain JSON Schema via `GenerateOptions.ResponseJSONSchema` (`video.RecipeSchema` returns `map[string]any`), not `genai.Schema`. `gemini.CleanJSONResponse` repairs model output before unmarshalling.
  - `imagegen` — keyframe / still-image generation (`imagegen.New(client)` → `*imagegen.Client`, satisfying `imagegen.Generator`). **References are `gs://` URIs only** — `Request.Images` is `[]string`, resolved server-side by Vertex AI, so no bytes are fetched, uploaded or cached and this repo needs no GCS reader, HTTP client or cache for images. There is no resolver to inject any more; a non-`gs://` reference is `ErrUnsupportedReference` and is the bug, not a reason to reach for a fetch path. If http(s) references ever become real, that is the signal to go back to `gemini-image-kit`, which still has the fetch / upload / compression surface.
  - `callguard` — the rate interval, per-call timeout and singleflight primitives behind `workflow/singleflight.go`.
  - `music` — `music.Recipe` (aliased as `video.MusicRecipe`) is the input music/lyrics recipe format. It is a dependency-free leaf, so importing it does not drag in the `lyria` workflow.
