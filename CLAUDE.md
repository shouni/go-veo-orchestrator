...
- `github.com/shouni/gemini-image-kit` — keyframe / still-image generation (`generator.New(client, resolver)` → `*generator.Generator`); this repo's `imagePorts.ImageGenerator` / `ImageRequest` / `ImageResponse` / `ImageURI` come from its `ports` package. **How a reference is sent is injected**, and `workflow.buildImageGenerator` injects `generator.NewGCSResolver()`: `gs://` references only, no bytes transferred, and the resolver itself takes no dependencies — which is why this repo needs no GCS reader, HTTP client or cache for images. This repo briefly used the `vertex-image-kit` fork instead, because the kit used to *require* a cache and fetch layer that never executed on the Vertex backend; v1.14 made those a resolver you opt into, so the fork lost its reason to exist. Do not add a `FetchResolver` to the chain to "fix" a rejected reference — a non-`gs://` reference is the bug.
